...
<%@ attribute name="limit" type="java.lang.Integer" required="false" description="每页大小" %>
<%@ attribute name="simple" type="java.lang.Boolean" required="false" description="是否简单风格" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>

<c:if test="${empty limit}">
    <c:set var="limit" value="${page.limit}"/>
</c:if>

<c:set var="displaySize" value="1"/>
...
<div class="pagination">
    <ul>
        <c:choose>
            ...
            <c:otherwise>
                <li><a href="#" onclick="$.table.turnPage('${limit}', 1, this);" title="首页">首页</a></li>
                <li><a href="#" onclick="$.table.turnPage('${limit}', ${current - 1}, this);" title="上一页">&lt;&lt;</a></li>
            </c:otherwise>
        </c:choose>

        <c:forEach begin="1" end="${begin == 1 ? 0 : 2}" var="i">
            <li <c:if test="${current == i}"> class="active"</c:if>>
                <a href="#" onclick="$.table.turnPage('${limit}', ${i}, this);" title="第${i}页">${i}</a>
            </li>
        </c:forEach>
        ...
        <c:forEach begin="${page.firstPage}" end="${end}" var="i">
            <li <c:if test="${current == i}"> class="active"</c:if>>
                <a href="#" onclick="$.table.turnPage('${limit}', ${i}, this);" title="第${i}页">${i}</a>
            </li>
        </c:forEach>
        ...
        <c:choose>
            ...
            <c:otherwise>
                <li><a href="#" onclick="$.table.turnPage('${limit}', ${current + 1}, this);" title="下一页">&gt;&gt;</a></li>
                <li><a href="#" onclick="$.table.turnPage('${limit}', ${page.totalPage}, this);" title="尾页">尾页</a></li>
            </c:otherwise>
        </c:choose>
    ...
    <div>
        <span class="page-input">
            第<input type="text" class="input-mini" value="${current}" onblur="$.table.turnPage('${limit}', $(this).val(), this);"/>页
        </span>
        &nbsp;
        <select class="input-small" onchange="$.table.turnPage($(this).val(), ${current}, this);">
            <option value="10" <c:if test="${limit eq 10}">selected="selected" </c:if>>10</option>
            <option value="20" <c:if test="${limit eq 20}">selected="selected" </c:if>>20</option>
            <option value="30" <c:if test="${limit eq 30}">selected="selected" </c:if>>30</option>
            <option value="50" <c:if test="${limit eq 50}">selected="selected" </c:if>>50</option>
        </select>
