...
<%@ attribute name="menu" type="cn.vansky.framework.core.web.easyUI.model.EasyUITreeModel" required="true" description="当前菜单" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
<%@taglib prefix="es" tagdir="/WEB-INF/tags" %>
<c:choose>
    <c:when test="${!menu.hasChildren}">
        <li><a href="<%=menuUrl(request, menu.getUrl())%>">${menu.text}</a></li>
    </c:when>
    <c:otherwise>
        <li>
            <a href="#">${menu.text}</a>
