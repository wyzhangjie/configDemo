...
<div data-table="table" class="panel">
    ...
    <ul class="nav nav-tabs">
        <li ${empty param['search.show_eq'] ? 'class="active"' : ''}>
            <a href="${ctx}/permission/role">
                <i class="icon-table"></i>
                所有角色列表
            </a>
        </li>
        <li ${param['search.show_eq'] eq true ? 'class="active"' : ''}>
            <a href="${ctx}/permission/role?search.show_eq=true">
                <i class="icon-table"></i>
                可用角色列表
            </a>
        </li>
        <li ${param['search.show_eq'] eq false ? 'class="active"' : ''}>
            <a href="${ctx}/permission/role?search.show_eq=false">
                <i class="icon-table"></i>
               不可用角色列表
            </a>
        </li>
...
<script type="text/javascript">
    $(function() {
        $(".status-show,.status-hide").click(function() {
            ...
            var url = isShow ?
                    "${ctx}/permission/role/changeStatus/true?" + checkbox.serialize()
                    :
                    "${ctx}/permission/role/changeStatus/false?" + checkbox.serialize();
            $.app.confirm({
                title : title,
                message : message,
                ok : function() {
        ...
        $.app.toggleLoadTable($("#table"), "${ctx}/permission/role/{parentId}/permissions")
    });
</script>
